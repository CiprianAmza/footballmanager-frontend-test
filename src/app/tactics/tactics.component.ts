...
export class TacticsComponent implements OnInit, OnChanges {
   ...
   *  The pitch is drawn with the own goal/GK at the BOTTOM (row 5) and strikers at
   *  the TOP (row 0), so "Deep" sits low near our goal and "High" pushes up the pitch.
   *  Presentation-only; does not touch the saved tactic. */
  get defLineTopPercent(): number {
    // Sit at the back line (Standard) so it reads as the defensive line; High pushes it up a band,
    // Deep drops it toward our goal. Defenders render around ~68% from the top.
    return this.selectedOptions.defensiveLine === 'High' ? 56
         : this.selectedOptions.defensiveLine === 'Deep' ? 76
         : 68;
  }

  /** Horizontal shift (% of a cell) applied to a pitch cell so WIDTH visibly spreads/compresses the
   *  XI: 'Wide' pushes the flank columns outward, 'Narrow' pulls them toward the centre. Driven by the
   *  cell's column (index % 5) distance from the central column (2). Presentation-only. */
   ...
   *  'High' lifts the back line + DM toward the opponent (up = negative), 'Deep' drops them toward
   *  our goal; attackers barely move and the GK only a touch. Presentation-only. */
