...
export class MultiplayerRoomComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void { this.sub = timer(1000, 1000).pipe(exhaustMap(() => this.room.state().pipe(catchError(e => { if (e.status !== 404) this.error = this.typedError(e); return EMPTY; })))).subscribe(s => { this.state = s; this.countdown = this.remaining(); }); }
  ...
  private applyState(s: MultiplayerState): void { this.state = s; this.settings = { continueThresholdPercent: s.continueThresholdPercent, dayTimeoutSeconds: s.dayTimeoutSeconds, majorityTimeoutSeconds: s.majorityTimeoutSeconds, maxPlayers: s.maxPlayers }; }
