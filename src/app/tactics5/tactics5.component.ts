...
interface Player {
    ...
    condition?: number;
    sharpness?: number;
    // Procedural-face descriptor fields (from the backend PlayerView); optional
    // because not every endpoint populates them — bound with `|| 0` fallbacks.
    baseFaceId?: number;
    skinTone?: number;
    hairStyle?: number;
    hairColor?: number;
...
@Component({
  selector: 'app-tactics5',
  templateUrl: './tactics5.component.html',
  styleUrls: ['./tactics5.component.css']
})
export class Tactics5Component implements OnInit, OnChanges {
  ...
  // ===== UI-state only (wizard stepper) — no data/save behaviour change =====
  currentStep: number = 0; // 0=Formation, 1=Squad, 2=Style, 3=Review
  readonly steps: { key: string; label: string; sub: string }[] = [
    { key: 'formation', label: 'Formation', sub: 'Shape & system' },
    { key: 'squad',     label: 'Squad',     sub: 'Assign players' },
    { key: 'style',     label: 'Style',     sub: 'Team instructions' },
    { key: 'review',    label: 'Review',    sub: 'Confirm & save' }
  ];
  ...
  nextStep(): void { if (this.currentStep < this.steps.length - 1) this.currentStep++; }
  prevStep(): void { if (this.currentStep > 0) this.currentStep--; }
  isStepDone(i: number): boolean { return i < this.currentStep; }

  // Pure display getters (no side effects)
  get assignedCount(): number {
    return this.fieldPositions.filter(p => p.player).length;
  }
  get subsCount(): number {
    return this.substitutes.filter(p => p.player).length;
  }
  get lineupComplete(): boolean {
    return this.assignedCount >= 11;
  }
  get progressPercent(): number {
    return Math.round(((this.currentStep + 1) / this.steps.length) * 100);
  }
  // Players on the pitch, top-rated first, for the review summary
  get startingEleven(): Player[] {
  ...
  /** Vertical position (% from top) of the blueprint defensive-line overlay.
   *  The grid renders strikers at the TOP (index 0) and the GK/own goal at the BOTTOM (index 29),
   *  so 'Deep' hugs the bottom (near own goal) and 'High' pushes up toward the opponent.
   *  Presentation-only. */
